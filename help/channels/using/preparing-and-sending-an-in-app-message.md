...
1. Renseignez les propriétés du message in-app et sélectionnez votre application mobile dans le champ **[!UICONTROL Associer une application mobile à une diffusion]**.
   ...
   Si aucune application n&#39;est visible dans la liste déroulante, assurez-vous que l&#39;état de vos applications mobiles soit défini sur **Configuré**. Les applications dont l&#39;état est **Prêt à être configuré** n&#39;apparaissent pas dans la liste. Pour plus d&#39;informations sur cette configuration d&#39;application mobile, consultez cette [page](../../administration/using/configuring-a-mobile-application.md#channel-specific-config).
...
1. Si vous utilisez un **[!UICONTROL Événements Analytics]**, les événements Adobe Analytics et Etat d’affichage seront automatiquement renseignés en fonction des suites de rapports configurées dans l’extension Analytics dans l’interface utilisateur de la collecte de données, tandis que les événements Données de contexte devront être ajoutés manuellement.
...
1. Après lʼenvoi de vos messages in-app, vous pouvez choisir de désactiver votre diffusion. Cela peut s’avérer utile si vous souhaitez arrêter une diffusion particulière ou si vous souhaitez exécuter une nouvelle diffusion avec le même déclencheur, par exemple.

   Cliquez sur **[!UICONTROL Désactiver]** puis **[!UICONTROL OK]** pour démarrer la requête de désactivation.

   ![](assets/inapp_sending_8.png)

1. Une fois la requête envoyée, votre diffusion sera désactivée et aucun autre message ne sera envoyé.
