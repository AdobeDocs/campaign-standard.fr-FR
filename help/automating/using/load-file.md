...
1. Depuis l'onglet **[!UICONTROL Structure du fichier], visualisez la structure automatiquement détectée du fichier d'exemple.**
   ...
   * Vous pouvez modifier les paramètres de détection par défaut afin de les adapter à votre fichier. Le champ **[!UICONTROL Type de fichier]permet d'indiquer si le fichier à importer est composé de colonnes à longueur fixe.** Si c'est le cas, vous devez également spécifier le nombre maximal de caractères pour chaque colonne dans l'onglet **[!UICONTROL Définition des colonnes].**
      ...
      Dans la section **[!UICONTROL Format du fichier]**, l'option **Vérifier les colonnes du fichier par rapport aux définitions de colonne]vous permet de vérifier que les colonnes du fichier que vous téléchargez correspondent à la définition des colonnes.[!UICONTROL **
...
1. Depuis l'onglet **[!UICONTROL Définition des colonnes], vérifiez le format des données de chaque colonne et ajustez les paramètres si nécessaire.**
   ...
   Par exemple, vous pouvez modifier le libellé d'une colonne, sélectionner son type (chaîne, entier, date, etc.) ou même spécifier le traitement d'erreur.
...
* **[!UICONTROL Format et séparateurs]**, **Propriétés**: spécifiez les propriétés d'un texte, le format d'heure, de date et de valeur numérique, ainsi que le séparateur spécifié par le contexte de la colonne.
   ...
   * **[!UICONTROL Format d'heure]**, **[!UICONTROL format de date]**: spécifiez le format des **données Date**, **Heure** et **Date et heure** .
...
* **[!UICONTROL Traitement des erreurs]** : permet de définir le comportement en cas d'erreur.
   ...
   * **[!UICONTROL Utiliser une valeur par défaut en cas d'absence de recodification]** : remplace la valeur causant l'erreur par une valeur par défaut, définie dans le champ **[!UICONTROL Valeur par défaut]**, sauf si une recodification a été définie pour la valeur en erreur (voir option **Recodification des valeurs]ci-dessus).[!UICONTROL **
...
1. Placez une activité **[!UICONTROL Chargement de fichier]** et connectez-la à la suite de l'activité **Transfert de fichier[!UICONTROL .]**
1. Sélectionnez l'activité puis ouvrez-la à l'aide du bouton ![, disponible dans les actions rapides qui s'affichent.](assets/edit_darkgrey-24px.png)
1. Dans la section **[!UICONTROL Fichier à charger]** de l'onglet **[!UICONTROL Exécution], cochez la case** Utiliser le fichier provenant de la transition entrante **.**

   ![](assets/wkf_file_loading8.png)

1. Paramétrez l'activité comme indiqué ci-dessus.
1. Placez une activité **[!UICONTROL Mise à jour de données]**, connectez-la à la suite de l'activité **Chargement de fichier]et paramétrez-la.[!UICONTROL ** Voir [Mise à jour de données](../../automating/using/update-data.md).
