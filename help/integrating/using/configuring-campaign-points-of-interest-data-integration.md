...
1. Ensuite, dans l’interface utilisateur de collecte de données, vous devez créer des règles pour prendre en charge les cas pratiques mobiles entre Point ciblé et Adobe Campaign.\
...
* L’URL **[!UICONTROL Point d’entrée pour la collecte des PII]** sert à collecter les jetons d’enregistrement et les identifiants Experience Cloud des utilisateurs à partir de l’application mobile lors de son lancement. Lorsqu’un utilisateur ou une utilisatrice se connecte à l’application à l’aide d’informations d’identification comme l’adresse e-mail, le prénom et le nom, ces données sont également utilisées pour réconcilier le jeton d’enregistrement de l’utilisateur ou de l’utilisatrice avec un profil Adobe Campaign.
