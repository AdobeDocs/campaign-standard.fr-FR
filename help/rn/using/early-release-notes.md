...
<tr> 
<td>
<p>Le service de commentaires par email (EFS) est un service évolutif qui améliore la précision des rapports en capturant les commentaires par email directement issus du MTA amélioré.</p>
<ul>
<li>Toutes les catégories d’événements sont capturées : retards, diffusions, envois, désabonnements (lien, liste), commentaires (plaintes liées au spam, événements asynchrones).</li>
<li>Le calcul des indicateurs envoyés/délivrés est désormais fondé sur les commentaires en temps réel du MTA amélioré, ce qui contribue à l'amélioration de la précision et de la réactivité.</li>
<li>Le service de commentaires par email (EFS) résout le problème des retards de reporting des bounces synchrones et diminue de 80 % la charge du processus inMail.</li>
</ul>
<p>Cette fonctionnalité est disponible en version <strong>bêta privée</strong> et sera progressivement disponible pour tous les clients dans les prochaines versions.</p>
</td> 
</tr> 
</tbody> 
</table>
...
* Une nouvelle fonction **GetOption** est désormais disponible dans les activités utilisant des variables d’événement après avoir appelé un workflow avec des paramètres externes. Elle vous permet de renvoyer la valeur d’une fonction spécifiée.
...
* L’activité **Transfert de fichier** génère désormais une variable supplémentaire (filesCount) qui contient le nombre de fichiers transférés ou téléchargés. (CAMP-45842)
...
* Correction d’un problème en raison duquel l’analyse de diffusion échouait lors de l’envoi d’un message push transactionnel utilisant la dimension cible Profil. Un nouveau mapping de diffusion (mapRtEventAppSubRcp) est désormais disponible pour les messages push transactionnels ciblant les profils. Les logs de diffusion, d’exclusion et de tracking de ces diffusions seront désormais disponibles dans les tables broadLogAppSubRcp, excludeLogAppSubRcp et trackingLogAppSubRcp.
...
* Correction d’une erreur qui pouvait vous empêcher la sélection d&#39;une collection avec une cible de ressources incorrecte dans une activité de requête de workflow.
...
* Correction d’une régression introduite dans la version 20.4 qui pouvait entraîner des taux d’ouverture incorrects pour Gmail en raison du filtrage des événements de tracking reçus de comptes Gmail. (CAMP-46504)
...
* Correction d’un problème qui pouvait ralentir l’affichage des rendus dans le rapport Rendu des emails. (CAMP-46226)
